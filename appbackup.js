...
function handleFileUpload(event) {
    ...
    reader.onload = function (e) {
        ...
        const sheet = workbook.Sheets["Inventário ", NomeCliente];
        if (!sheet) {
            alert("Aba 'Inventário SONEPAR", NomeCliente,"não encontrada.");
            return;
        }
...
function applyFilters() {
    ...
    filtered = data.filter(row => {
        ...
        // Aplica filtro do gráfico (COMBINADO com filtros acima)
        if (chartFilterActive) {
            const fieldValue = row[chartFilterActive.field];
            if (Array.isArray(fieldValue)) {
                if (!fieldValue.some(v => normalizeText(v) === chartFilterActive.value)) return false;
            } else {
                if (chartFilterActive.field === "sensivel" || chartFilterActive.field === "menor") {
                    if (!((fieldValue && chartFilterActive.value === "sim") || (!fieldValue && chartFilterActive.value === "não"))) return false;
                } else {
                    if (normalizeText(fieldValue) !== chartFilterActive.value) return false;
                }
            }
        }
...
function updateFilterIndicator() {
    ...
    if (chartFilterActive) {
        const fieldLabel = {
            ...
            "empresa": "Empresa"
        }[chartFilterActive.field] || chartFilterActive.field;

        indicator.innerHTML = `<strong>Filtro ativo (gráfico):</strong> ${fieldLabel}: ${chartFilterActive.label}`;
