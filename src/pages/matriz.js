...
const RISK_COLUMN_MAP = {
    ...
    subprocesso: ["Sub-processo indicado"],
    classificacao: ["Classificação"],
    nomeRisco: ["Nome Risco"],
    descricao: ["Descrição do risco"],
    probabilidade: ["Probabilidade"],
    severidade: ["Severidade/Impacto"],
    nivelRisco: ["Nível de Risco (NR)"],
    estrategiaResposta: ["Estratégia de Resposta ao risco"],
    controlePreventivo: ["Estratégia de tratamento do risco \n(Controles Preventivo)"],
    controleDetectivo: ["Estratégia de tratamento do risco \n(Controles Detectivo)"],
    planoAcao: ["Plano de Ação"],
    custoAdicional: ["Custo Adicional?"],
    statusPlano: ["Status Plano de  Ação"],
    comentariosArea: ["Comentários da Área"],
    comentariosConsultoria: ["Comentários Consultoria"],
    evidenciaEsperada: ["Evidência esperada"],
    evidenciaPadronizada: ["Evidencia padronizada"],
    atividadeAdequacao: ["Atividade relacionada do plano de adequação"]
};
...
function handleFileUploadMatriz(event) {
    ...
    reader.onload = function (e) {
        ...
        const json = parseSheetWithHeaderDetection(sheet, RISK_REQUIRED_FIELDS, RISK_COLUMN_MAP);

        if (!json || json.length === 0) {
            alert("A planilha de matriz foi carregada, mas não contém dados válidos.");
            return;
        }

        rawRiskData = json;

        const missingFields = listMissingColumns(rawRiskData[0], RISK_REQUIRED_FIELDS, RISK_COLUMN_MAP);
        if (missingFields.length > 0) {
            alert(`Campos obrigatórios ausentes na matriz de risco: ${missingFields.join(', ')}. Verifique o arquivo e tente novamente.`);
            rawRiskData = [];
            return;
        }
        ...
        const connectionNote = Array.isArray(data) && data.length > 0
...
function normalizeRiskData() {
    riskData = rawRiskData.map(row => {
        const nivelRawText = normalizeText(getRiskField(row, "nivelRisco"));
        let nivelRisco = "baixo";
        
        if (nivelRawText.includes("crítico") || nivelRawText.includes("critico")) {
            nivelRisco = "crítico";
        } else if (nivelRawText.includes("alto")) {
            nivelRisco = "alto";
        } else if (nivelRawText.includes("médio") || nivelRawText.includes("medio")) {
            nivelRisco = "médio";
        } else {
            nivelRisco = "baixo";
        }
        ...
        return {
            ...
            grupoRiscos: getRiskField(row, "grupoRiscos") || "",
            nomeGrupo: getRiskField(row, "nomeGrupo") || "",
            empresa: normalizeText(getRiskField(row, "empresa")) || "não informado",
            area: normalizeText(getRiskField(row, "area")) || "não informado",
            responsavel: getRiskField(row, "responsavel") || "—",
            referencia: getRiskField(row, "referencia") || "—",
            subprocesso: getRiskField(row, "subprocesso") || "—",
            classificacao: getRiskField(row, "classificacao") || "—",
            nomeRisco: getRiskField(row, "nomeRisco") || "",
            descricao: getRiskField(row, "descricao") || "—",
            probabilidade: normalizeText(getRiskField(row, "probabilidade")) || "não informado",
            severidade: normalizeText(getRiskField(row, "severidade")) || "não informado",
            nivelRisco: nivelRisco,
            estrategiaResposta: getRiskField(row, "estrategiaResposta") || "—",
            controlePreventivo: getRiskField(row, "controlePreventivo") || "—",
            controleDetectivo: getRiskField(row, "controleDetectivo") || "—",
            planoAcao: getRiskField(row, "planoAcao") || "—",
            custoAdicional: normalizeText(getRiskField(row, "custoAdicional")) || "não informado",
            statusPlano: normalizeText(getRiskField(row, "statusPlano")) || "não informado",
            comentariosArea: getRiskField(row, "comentariosArea") || "—",
            comentariosConsultoria: getRiskField(row, "comentariosConsultoria") || "—",
            evidenciaEsperada: getRiskField(row, "evidenciaEsperada") || "—",
            evidenciaPadronizada: getRiskField(row, "evidenciaPadronizada") || "—",
            atividadeAdequacao: getRiskField(row, "atividadeAdequacao") || "—"
        };
    }).filter(r => r.id); // Filtrar apenas registros com ID
}
...
function populateRiskFilterOptions() {
    const empresas = [...new Set(riskData.map(r => r.empresa).filter(e => e && e !== "não informado"))].sort();
    const areas = [...new Set(riskData.map(r => r.area).filter(a => a && a !== "não informado"))].sort();
...
function applyRiskFilters() {
    const fEmpresa = normalizeText(document.getElementById('risk-empresa')?.value || "");
    const fArea = normalizeText(document.getElementById('risk-area')?.value || "");
    const fNivel = normalizeText(document.getElementById('risk-nivel')?.value || "");
    ...
    filteredRisk = riskData.filter(row => {
        ...
        for (const [field, values] of Object.entries(riskChartFilters)) {
            ...
            const matches = values.some(value => matchesRiskFilterValue(field, rowValue, value));
...
function resetRiskFilters() {
    document.querySelectorAll('#risk-empresa, #risk-area, #risk-nivel').forEach(select => {
        select.value = "";
    });
...
function renderRiskKpis() {
    ...
    const criticos = filteredRisk.filter(r => r.nivelRisco === "crítico").length;
    const altos = filteredRisk.filter(r => r.nivelRisco === "alto").length;
    const medios = filteredRisk.filter(r => r.nivelRisco === "médio").length;

    const empresasUnicas = new Set(filteredRisk.map(r => r.empresa).filter(e => e !== "não informado")).size;
...
function renderRiskCharts() {
    ...
    filteredRisk.forEach(r => {
        if (r.probabilidade && r.probabilidade !== "não informado") {
            probabilidadeCount[r.probabilidade] = (probabilidadeCount[r.probabilidade] || 0) + 1;
        }
    });
    ...
    createRiskChart("chartRiskArea", topEmpresaArea, "bar");
}
...
function renderRiskTable() {
    ...
    tbody.innerHTML = filteredRisk.map(r => `
        <tr>
...
function renderRiskHeatmap() {
    ...
    if (!filteredRisk || filteredRisk.length === 0) {
        containers.forEach(container => {
            container.innerHTML = '<div style="padding: 24px; background: var(--surface2); border-radius: 8px; color: var(--text-muted); text-align: center;">Nenhum dado de risco disponível. Importe a matriz para ver o heatmap.</div>';
        });
        return;
    }
    ...
    heatmapHTML += '<div style="display: grid; grid-template-columns: 100px repeat(4, 1fr); gap: 8px; margin-bottom: 16px;">';
    heatmapHTML += '<div></div>';
    ...
    for (const prob of probabilidades) {
        heatmapHTML += '<div style="display: grid; grid-template-columns: 100px repeat(4, 1fr); gap: 8px; margin-bottom: 8px;">';
...
function renderMatrix() {
    ...
    let summary = '<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 24px;">';
...
function renderActionPlan() {
    ...
    if (actionItems.length === 0) {
        html += '<div style="padding: 16px; background: var(--surface2); border-radius: 8px; color: var(--text-muted); text-align: center;">Nenhum plano de ação encontrado</div>';
    } else {
        html += '<div style="display: grid; gap: 12px;">';
        actionItems.forEach(r => {
            const tasks = String(r.planoAcao || "").split(/\n|;/).map(task => task.trim()).filter(task => task);
            html += `
