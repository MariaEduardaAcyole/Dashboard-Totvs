...
const INVENTORY_REQUIRED_FIELDS = ["area", "processo", "dados"];
// =============================================
// UTILITÁRIOS
// =============================================
...
// =============================================
const COLUMN_MAP = {
  id: ["id", "ID", "Código", "ID Processo", "ID Subprocesso"],

  empresa: ["Empresa", "Nome da empresa", "Empresa/Unidade"],

  area: ["Nome da área de negócio", "Área de negócio", "Área", "Nome da Área"],
  ...
  processo: ["Nome do processo", "Processo"],

  subprocesso: ["Nome do subprocesso", "Subprocesso", "Sub-processo"],

  descricao: ["Descrição do subprocesso", "Descrição do sub-processo"],

  dados: ["Quais dados?", "Quais dados são tratados?", "Dados tratados"],

  sensivel: [
    "Utiliza dado sensível? Se o dado revela saúde, biometria, crença, política, origem racial, sindicato, genética ou vida sexual, trate como sensível.",
    "Dado Sensível?",
    "Dados Sensíveis",
    "Dado_Sensivel",
  ],
  ...
  menor: ["Dados de menores?", "Dado_Menor"],

  dadosMenores: ["Quais dados de menores?"],
  ...
  baseLegal: ["Base Legal", "Base_Legal"],

  terceiros: ["Compartilhamento Terceiros", "Compartilhamento_Terceiros"],

  nomeTerceiro: ["Nome Terceiro", "Nome_Terceiro"],

  transfIntl: ["Transferência Internacional", "Transferencia_Internacional"],
...
function handleFileUpload(event) {
  ...
  reader.onload = function (e) {
    ...
    if (!json || json.length === 0) {
      alert(
        "A planilha de inventário foi carregada, mas não contém dados válidos.",
      );
      return;
    }
    ...
    if (missingFields.length > 0) {
      alert(
        `Campos obrigatórios ausentes no inventário: ${missingFields.join(", ")}. Verifique o arquivo e tente novamente.`,
      );
      rawData = [];
      return;
    }
    ...
    console.log(
      "Colunas detectadas no inventário:",
...
function normalizeData() {
  data = rawData
    .map((row, idx) => {
      ...
      let areaOriginal =
        getValue(row, "area") ||
        row["Nome da área de negócio"] ||
        row["Área"] ||
        "";
      let processoBruto =
        getValue(row, "processo") ||
        row["Nome do processo"] ||
        row["Processo"] ||
        "";
      let subprocessoBruto =
        getValue(row, "subprocesso") ||
        row["Nome do sub-processo"] ||
        row["Sub-processo"] ||
        "";
      ...
      if (typeof processoBruto === "string")
        processoBruto = processoBruto.trim();
      if (typeof subprocessoBruto === "string")
        subprocessoBruto = subprocessoBruto.trim();
      ...
      const menor =
        menorRaw === "sim" || menorRaw === "s" || menorRaw.includes("sim");
    ...
    .filter((r) => {
      ...
      const temArea =
        r.area.length > 0 && r.area[0] !== "" && r.area[0] !== "não informado";
      const temProcesso =
        r.processo.length > 0 &&
        r.processo[0] !== "" &&
        r.processo[0] !== "não informado";
...
function getRowKeyFromRaw(rawRow, index) {
  ...
  return normalizeText(
    String(rawRow[Object.keys(rawRow)[0]] || `__row_${index}`),
  );
}
...
function getRiskUniqueId(risk) {
  ...
  if (id) return id;
  return normalizeText(
    `${risk.empresa}|${risk.area}|${risk.subprocesso}|${risk.nomeRisco}`,
  );
}

function riskMatchesInventoryRow(inv, risk) {
  const invId = normalizeText(inv.id || "");

  const riskId = normalizeText(risk.idSubprocesso || "");
  ...
  const empresaMatch =
    normalizeText(inv.empresa?.[0]) === normalizeText(risk.empresa);

  const areaMatch = normalizeText(inv.area?.[0]) === normalizeText(risk.area);

  const subprocessoMatch =
    normalizeText(inv.subprocesso?.[0]) === normalizeText(risk.subprocesso);

  return empresaMatch && areaMatch && subprocessoMatch;
}
...
function buildInventoryRiskLinks() {
  ...
  data.forEach((inv) => {
    ...
    const matches = riskData.filter((risk) =>
      riskMatchesInventoryRow(inv, risk),
    );
    ...
    matches.forEach((risk) => {
      const id = getRiskUniqueId(risk);

      if (seen.has(id)) return;

      seen.add(id);

      uniqueMatches.push(risk);
    });

    const byLevel = uniqueMatches.reduce((acc, r) => {
      const nivel = r.nivelRisco || "Não classificado";
    ...
    inventoryRiskLinks[invKey] = {
      ...
      riskIds: uniqueMatches.map(getRiskUniqueId),
...
function applyFilters() {
  ...
  const fEmpresa = normalizeText(
    document.getElementById(filterPrefix + "empresa")?.value || "",
  );
  const fArea = normalizeText(
    document.getElementById(filterPrefix + "area")?.value || "",
  );
  const fProcesso = normalizeText(
    document.getElementById(filterPrefix + "processo")?.value || "",
  );
  const fSensivel =
    document.getElementById(filterPrefix + "sensivel")?.value || "";
  ...
  filtered = data.filter((row) => {
    ...
    for (const [field, values] of Object.entries(chartFilters)) {
      ...
      const matches = values.some((value) =>
        matchesFilterValue(field, fieldValue, value),
      );
...
function renderKPIs() {
  ...
  const total = filtered.reduce((sum, r) => sum + getSubprocessoCount(r), 0);
...
function groupSubprocessosBy(field) {
  return filtered.reduce((acc, row) => {
    ...
    groups.forEach((group) => {
      ...
      if (normalized === "não informado" || normalized === "nao informado")
        return;
...
// Score LGPD por subprocesso
function calculateRiskScore(row) {
  let score = 0;

  if (row.sensivel) score += 3;
  if (row.menor) score += 4;
  if (row.terceiros) score += 2;
  if (row.transfIntl) score += 2;

  return score;
}

// =============================================
// Ranking áreas por risco
// =============================================

function groupAreaRiskScore() {
  return filtered.reduce((acc, row) => {
    const area = row.area[0] || "Não informado";

    acc[area] = (acc[area] || 0) + calculateRiskScore(row);
...
// =============================================
// Compartilhamento externo por área
// =============================================

function groupThirdPartyByArea() {
  return filtered.reduce((acc, row) => {
    if (!row.terceiros) return acc;

    const area = row.area[0];

    acc[area] = (acc[area] || 0) + getSubprocessoCount(row);
...
function groupCriticalSystems() {
  return filtered.reduce((acc, row) => {
    if (!row.sensivel) return acc;

    row.sistemas.forEach((system) => {
      acc[system] = (acc[system] || 0) + getSubprocessoCount(row);
    });
...
// =============================================
// Dados sensíveis por processo
// =============================================

function sensitiveByProcess() {
  return filtered.reduce((acc, row) => {
    if (!row.sensivel) return acc;

    const processo = row.processo[0];

    acc[processo] = (acc[processo] || 0) + getSubprocessoCount(row);
...
// =============================================
// Riscos relacionados por área
// =============================================

function relatedRisksByArea() {
  return filtered.reduce((acc, row) => {
    const area = row.area[0];

    const key = getInventoryRowKey(row);

    const total = inventoryRiskLinks[key]?.total || 0;

    acc[area] = (acc[area] || 0) + total;
...
// =============================================
// Base legal x subprocessos
// =============================================

function baseLegalSubprocessos() {
  return filtered.reduce((acc, row) => {
    row.baseLegal.forEach((base) => {
      acc[base] = (acc[base] || 0) + getSubprocessoCount(row);
    });
...
function maturityIndex() {
  ...
  filtered.forEach((row) => {
    ...
    if (row.baseLegal.length) score++;

    if (!row.terceiros) score++;

    if (!row.transfIntl) score++;

    if (!row.sensivel) score++;

    if (score >= 4) result.Alto++;
    else if (score >= 2) result.Médio++;
    else result.Baixo++;
  });
...
function storageDistribution() {
  return filtered.reduce((acc, row) => {
    const value = row.armazenamento || "Não informado";
...
function groupDataCategories() {
  ...
  filtered.forEach((row) => {
    const dados = row.dados.join(" ").toLowerCase();

    if (row.sensivel) result["Dados sensíveis"]++;

    if (row.menor) result["Dados menores"]++;

    if (dados.includes("cpf") || dados.includes("nome")) {
      result["Dados pessoais"]++;
    }

    if (dados.includes("salário") || dados.includes("cartão")) {
      result["Financeiros"]++;
    }

    if (dados.includes("digital") || dados.includes("biometr")) {
      result["Biométricos"]++;
    }
  });
...
function renderLgpdCharts() {
  createChart("chartAreaRisk", sortByValue(groupAreaRiskScore()), "bar");

  createChart(
    "chartThirdPartyArea",
    sortByValue(groupThirdPartyByArea()),
    "bar",
  );

  createChart(
    "chartCriticalSystems",
    topN(sortByValue(groupCriticalSystems()), 10),
    "bar",
    "y",
  );

  createChart(
    "chartSensitiveProcess",
    sortByValue(sensitiveByProcess()),
    "bar",
  );

  createChart("chartRelatedRiskArea", sortByValue(relatedRisksByArea()), "bar");

  createChart(
    "chartBaseLegalSub",
    sortByValue(baseLegalSubprocessos()),
    "doughnut",
  );

  createChart("chartMaturity", maturityIndex(), "doughnut");

  createChart("chartStorage", storageDistribution(), "doughnut");

  createChart("chartDataCategories", groupDataCategories(), "doughnut");
}
...
function renderTable() {
  ...
  if (!rawData || rawData.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="999">Nenhum dado do inventário carregado.</td></tr>';
  ...
  const displayProcessed = filtered.filter((r) => {
    const rawRow =
      ...
      rawData.find((row) => {
        ...
        return (
          String(row[firstKey]) === String(r.id) ||
          String(getValue(row, "id")) === String(r.id)
        );
      });
  ...
  tbody.innerHTML = rows
    .map((row, index) => {
      ...
      const riskDetail =
        riskLink.total > 0
          ? `${riskLink.total} risco(s) — ${Object.entries(riskLink.byLevel)
      ...
      return (
        ...
        rawHeaders
          .map(
            (h) =>
              `<td>${row[h] !== undefined && row[h] !== null ? row[h] : ""}</td>`,
          )
          .join("") +
        `<td>${riskDetail}</td>` +
        "</tr>"
...
function renderCharts() {
  createChart("chartArea", sortByValue(groupSubprocessosBy("area")), "bar");
...
function hora() {
  var campo = document.getElementById("lastUpdate");
  console.log(campo);
  campo.textContent = Date();
}
